...
B = [b1;
     ...
     b3];
%% Initialization
p33_0 = [0.5 0.1 0.05 0.01]; 
for jj = 1:length(p33_0)
u(1) = it(1);
x_hat(:,1) = [0;0;0.5];
...
P{1} = [0.001 0 0;0 0.001 0;0 0 p33_0(jj)];
R = 0.2;
Q = 1e-6*eye(3);
In = eye(size(A,1));
...
legend({"Measured","$P_{0,0}(3,3) = 0.50$","$P_{0,0}(3,3) = 0.10$",...
       "$P_{0,0}(3,3) = 0.05$","$P_{0,0}(3,3) = 0.01$"},"interpreter",...
       "latex","fontsize",14)
axes('Position',[.24 .25 .30 .30])
indexOfInterest = (t >= 0) & (t <= 500);
...
xlabel("Time (s)","Interpreter","latex","FontSize",20)
legend({"$P_{0,0}(3,3) = 0.50$","$P_{0,0}(3,3) = 0.10$",...
       "$P_{0,0}(3,3) = 0.05$","$P_{0,0}(3,3) = 0.01$"},"interpreter",...
       "latex","fontsize",14)
axes('Position',[.24 .45 .30 .30])
indexOfInterest = (t >= 0) & (t <= 500);
